...
async function ledOn(pin) {
  ...
  if (pin === 1) {
    LED14.writeSync(1);
    //!confirming wheather the light has turned on or not
    let a = pinStatus(1);
...
process.on("SIGINT", () => {
  console.log("Keyboard interrupt Received SIGINT.  exit.........");
  process.exit(0);
});
process.on("SIGTERM", () => {
  console.log("TERM SIGNAL Received .  exit.........");
  process.exit(0);
});
