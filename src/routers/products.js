...
router.get("/products/:id", [productFilter], async (req, res) => {
  try {
...
router.get("/search/:name", async (req, res) => {
  try {
    ...
    let item = data.map((items) => {
      items.map((item) => item.title.includes(req.params.name));
    });

    console.log(item);
    res.status(200).send(`you searched :${req.params.name}`);
  } catch (error) {
    console.log(error);
